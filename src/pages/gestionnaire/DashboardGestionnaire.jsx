...
//   // Fonction pour normaliser les types de déchets
//   const normaliserType = (type) => {
//     if (!type) return '';
//     // Convertir en minuscules et remplacer les underscores par des espaces
//     return type.toLowerCase().replace(/_/g, ' ').trim();
//   };

//   // Vérifier si un type de mission correspond à un type de campagne
//   const typeCorrespond = (typeMission, typeCampagne) => {
//     const missionNorm = normaliserType(typeMission);
//     const campagneNorm = normaliserType(typeCampagne);
    
//     console.log(`Comparaison: mission="${missionNorm}" (${typeMission}) vs campagne="${campagneNorm}" (${typeCampagne})`);
    
//     // Correspondance exacte
//     if (missionNorm === campagneNorm) return true;
    
//     // Cas spéciaux
//     if (campagneNorm.includes('plastique') && missionNorm.includes('plastique')) return true;
//     if (campagneNorm.includes('papier') && missionNorm.includes('papier')) return true;
//     if (campagneNorm.includes('carton') && missionNorm.includes('carton')) return true;
//     if (campagneNorm.includes('metal') && missionNorm.includes('metal')) return true;
//     if (campagneNorm.includes('verre') && missionNorm.includes('verre')) return true;
...
//       const response = await fetch('http://localhost:3000/api/campagnes?statut=planifiee', {
//         headers: { 'Authorization': `Bearer ${token}` }
//       });
//       const data = await response.json();
...
//         }));
//         console.log('✅ Campagnes formatées:', campagnesFormatees);
...
//                       </p>
//                       <p className="text-sm text-blue-600">
//                         Type: <span className="font-semibold">{selectedMission.type_dechet}</span> | Quantité estimée: {selectedMission.quantite} {selectedMission.unite}
//                       </p>
//                     </div>
//                   </div>
//                 </div>
...
//                   {/* Afficher le nombre de campagnes disponibles */}
//                   <div className="text-xs text-gray-500 mb-1">
//                     {campagnes.length} campagne(s) active(s) au total
//                   </div>
...
//                     {campagnes
//                       .filter(c => {
//                         // Vérifier si le type de la mission correspond à un type accepté par la campagne
//                         const correspond = c.types_dechets?.some(typeCampagne => 
//                           typeCorrespond(typeCampagne, selectedMission.type_dechet)
//                         );
//                         console.log(`Campagne ${c.nom} (${c.types_dechets}) pour type ${selectedMission.type_dechet}: ${correspond ? '✅' : '❌'}`);
//                         return correspond;
//                       })
//                       .map(campagne => {
//                         const progression = campagne.poids_attendue ? 
...
//                   {campagnes.length > 0 && campagnes.filter(c => 
//                     c.types_dechets?.some(type => typeCorrespond(type, selectedMission.type_dechet))
//                   ).length === 0 && (
//                     <div className="mt-2 text-sm text-yellow-600 bg-yellow-50 p-2 rounded-lg">
//                       ⚠️ Aucune campagne active n'accepte le type de déchet "{selectedMission.type_dechet}"
//                     </div>
//                   )}
...
const DashboardGestionnaire = () => {
  ...
  // Ajoutez cette fonction getTypeLabel si elle n'existe pas dans le composant
...
const MissionsPage = () => {
  ...
  const [campagnes, setCampagnes] = useState([]);
  const [loadingCampagnes, setLoadingCampagnes] = useState(false);
  const [selectedCampagneDetails, setSelectedCampagneDetails] = useState(null);
...
const loadCampagnes = async () => {
  ...
  try {
    console.log('📡 Chargement des campagnes...');
    // Charger les campagnes actives avec leurs objectifs
    const response = await fetch('https://ecobackend-zeds.vercel.app/api/campagnes?statut=planifiee', {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    const data = await response.json();
    console.log('📦 Campagnes reçues:', data);
    ...
    if (data.success) {
      // Pour chaque campagne, charger les détails pour avoir les objectifs
      const campagnesAvecDetails = await Promise.all(
        (data.campagnes || []).map(async (c) => {
          try {
            // CORRECTION : Enlever le guillemet simple en trop
            const detailsRes = await fetch(`https://ecobackend-zeds.vercel.app/api/campagnes/${c.id}`, {
              headers: { 'Authorization': `Bearer ${token}` }
            });
            
            // Vérifier si la réponse est OK avant de parser le JSON
            if (!detailsRes.ok) {
              console.error(`Erreur HTTP ${detailsRes.status} pour la campagne ${c.id}`);
              return c;
            }
            
            const detailsData = await detailsRes.json();
            return detailsData.success ? detailsData.campagne : c;
          } catch (error) {
            console.error(`Erreur chargement détails campagne ${c.id}:`, error);
            return c;
          }
        })
      );
      
      console.log('✅ Campagnes avec objectifs:', campagnesAvecDetails);
      setCampagnes(campagnesAvecDetails);
    } else {
      setCampagnes([]);
    }
  } catch (error) {
  ...
  // Fonction pour obtenir l'objectif d'une campagne pour un type de déchet spécifique
  const getObjectifPourType = (campagne, typeDechet) => {
    return campagne.objectifs?.find(obj => obj.type_dechet === typeDechet);
  };
  ...
  // Mettre à jour les détails quand on sélectionne une campagne
  useEffect(() => {
    if (validationForm.campagneId && selectedMission) {
      const campagne = campagnes.find(c => c.id === validationForm.campagneId);
      if (campagne) {
        const objectif = getObjectifPourType(campagne, selectedMission.type_dechet);
        setSelectedCampagneDetails({
          campagne,
          objectif
        });
      } else {
        setSelectedCampagneDetails(null);
      }
    } else {
      setSelectedCampagneDetails(null);
    }
  }, [validationForm.campagneId, selectedMission, campagnes]);
                    ...
                    <button
                      onClick={() => {
                        setValidationForm({
                          ...validationForm,
                          missionId: mission.id,
                          campagneId: '' // Réinitialiser la sélection
                        });
                        setSelectedMission(mission);
                        setSelectedCampagneDetails(null);
                        setShowValidationModal(true);
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            {!showConfirmation ? (
              <form className="p-6 space-y-6" onSubmit={(e) => e.preventDefault()}>
                ...
                {/* ✅ Sélection de campagne avec objectifs */}
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-gray-700">
                    Campagne (optionnel)
                  </label>
                  ...
                  >
                    ...
                    {campagnes
                      .filter(c => {
                        // Vérifier si la campagne a un objectif pour ce type de déchet avec du stock restant
                        const objectif = getObjectifPourType(c, selectedMission.type_dechet);
                        if (!objectif) return false;
                        
                        const reste = objectif.poids_attendue - (objectif.poids_collecte_actuel || 0);
                        return reste > 0;
                      })
                      .map(campagne => {
                        const objectif = getObjectifPourType(campagne, selectedMission.type_dechet);
                        const reste = objectif.poids_attendue - (objectif.poids_collecte_actuel || 0);
                        const progression = objectif.poids_attendue ? 
                          ((objectif.poids_collecte_actuel || 0) / objectif.poids_attendue * 100).toFixed(1) : 0;
                        
                        return (
                          <option key={campagne.id} value={campagne.id}>
                            {campagne.nom} - {objectif.type_dechet}: {reste.toFixed(1)} kg restant(s) sur {objectif.poids_attendue} kg à {objectif.prix_par_kg} FCFA/kg
                          </option>
                        );
                      })}
                  </select>

                  {/* Affichage des détails de l'objectif sélectionné */}
                  {selectedCampagneDetails && (
                    <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                      <h4 className="font-medium text-green-800 mb-3">Détails de l'objectif</h4>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-green-700">Type de déchet:</span>
                          <span className="font-medium text-gray-900">
                            {getTypeLabel(selectedCampagneDetails.objectif.type_dechet)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-green-700">Prix convenu:</span>
                          <span className="font-medium text-gray-900">
                            {selectedCampagneDetails.objectif.prix_par_kg} FCFA/kg
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-green-700">Objectif:</span>
                          <span className="font-medium text-gray-900">
                            {selectedCampagneDetails.objectif.poids_attendue} kg
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-green-700">Déjà collecté:</span>
                          <span className="font-medium text-green-600">
                            {selectedCampagneDetails.objectif.poids_collecte_actuel || 0} kg
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-green-700">Reste à collecter:</span>
                          <span className="font-medium text-orange-600">
                            {(selectedCampagneDetails.objectif.poids_attendue - (selectedCampagneDetails.objectif.poids_collecte_actuel || 0)).toFixed(1)} kg
                          </span>
                        </div>
                        ...
                        <div className="mt-2">
                          <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                            <div 
                              ...
                              style={{ 
                                width: `${Math.min(
                                  ((selectedCampagneDetails.objectif.poids_collecte_actuel || 0) / 
                                   selectedCampagneDetails.objectif.poids_attendue) * 100, 
                                  100
                                )}%` 
                              }}
                            />
                  ...
                  {campagnes.length > 0 && campagnes.filter(c => {
                    ...
                    <div className="mt-2 text-sm text-yellow-600 bg-yellow-50 p-2 rounded-lg">
                      ⚠️ Aucune campagne avec objectif disponible pour le type de déchet "{selectedMission.type_dechet}"
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {selectedCampagneDetails && (
                      <p className="text-xs text-gray-500 mt-1">
                        Prix de la campagne: {selectedCampagneDetails.objectif.prix_par_kg} FCFA/kg
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-600">Type déchet</span>
                    <span className="font-medium">{selectedMission.type_dechet}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Poids</span>
                    <span className="font-medium">{validationForm.poidsDepose} kg</span>
                  ...
                  {selectedCampagneDetails && (
                    <>
                      <div className="flex justify-between pt-2 border-t border-gray-200">
                        <span className="text-gray-800 font-semibold">Campagne</span>
                        <span className="text-green-600 font-medium">
                          {selectedCampagneDetails.campagne.nom}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600">Type campagne</span>
                        <span className="text-gray-800">
                          {getTypeLabel(selectedCampagneDetails.objectif.type_dechet)}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600">Prix campagne</span>
                        <span className="text-gray-800">
                          {selectedCampagneDetails.objectif.prix_par_kg} FCFA/kg
                        </span>
                      </div>
