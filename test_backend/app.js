...
//     console.log('Chargement du profil avec utilisateur:', currentUser);
    
//     // Afficher les informations personnelles complètes
...
//     document.getElementById('profileAddress').textContent = currentUser.adresse || 'Non spécifiée';
//     document.getElementById('profileLocation').textContent = `${currentUser.quartier || ''} ${currentUser.commune ? ', ' + currentUser.commune : ''}`.trim() || 'Non spécifié';
...
//     // Statut du compte - CORRECTION ICI
//     const statusElement = document.getElementById('profileStatus');
//     if (statusElement) {
//         if (currentUser.statut === 'actif' || currentUser.statut === 'Actif' || currentUser.estActif === true) {
//             statusElement.textContent = 'Actif';
//             statusElement.className = 'info-value badge badge-success';
//         } else {
//             statusElement.textContent = currentUser.statut || 'Actif';
//             statusElement.className = 'info-value badge badge-success';
//         }
//     }
    
//     // Afficher les informations de localisation détaillées
//     if (currentUser.latitude && currentUser.longitude) {
//         const locationInfo = document.createElement('div');
//         locationInfo.className = 'info-item';
//         locationInfo.innerHTML = `
//             <span class="info-label">Coordonnées GPS:</span>
//             <span class="info-value">${currentUser.latitude}, ${currentUser.longitude}</span>
//         `;
//         document.querySelector('.info-grid').appendChild(locationInfo);
//     }
...
//         const response = await fetch(`${CONFIG.API_URL}/api/profil`, {
//             method: 'PUT',
//             headers: {
//                 'Content-Type': 'application/json',
//                 'Authorization': `Bearer ${currentToken}`
...
//         const response = await fetch(`${CONFIG.API_URL}/api/changer-mot-de-passe`, {
//             method: 'POST',
//             headers: {
//                 'Content-Type': 'application/json',
//                 'Authorization': `Bearer ${currentToken}`
...
//             // CORRECTION : Vérifier que total_dechets_collectes est un nombre avant d'appeler toFixed
//             let totalWaste = data.total_dechets_collectes;
//             if (totalWaste !== undefined && totalWaste !== null) {
//                 // Convertir en nombre si c'est une chaîne
//                 if (typeof totalWaste === 'string') {
...
//             currentToken = token;
//             currentUser = JSON.parse(userJson);
//             console.log('Utilisateur chargé du stockage:', currentUser);
//             updateUIForLoggedInUser();
//             showSection('dashboard');
//         } catch (error) {
//             console.error('Erreur lors du parsing des données utilisateur:', error);
...
async function handleLogin(e) {
    ...
    try {
        ...
        // Extraire l'utilisateur (peut être dans différentes propriétés)
...
async function handleForgotPassword(e) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Erreur lors de la demande' }));
            throw new Error(errorData.message || 'Erreur lors de la demande');
        }
        
        const data = await response.json();
        
        showSuccess(messageDiv, data.message || 'Email envoyé avec succès');
        document.getElementById('resetPasswordSection').classList.remove('hidden');
        resetForm('forgotPasswordForm');
...
async function handleResetPassword(e) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Erreur lors de la réinitialisation' }));
            throw new Error(errorData.message || 'Erreur lors de la réinitialisation');
        }
        
        const data = await response.json();
        
        showSuccess(messageDiv, data.message || 'Mot de passe réinitialisé avec succès');
        resetForm('resetPasswordForm');
        showTab('login');
...
async function handleNewDeclaration(e) {
    ...
    try {
        ...
        showSuccess(messageDiv, data.message || 'Déclaration créée avec succès!');
        hideNewDeclarationForm();
        loadDeclarations();
        loadDashboard();
...
async function showDeclarationDetail(declarationId) {
    ...
    try {
        ...
        content.innerHTML = createDetailHTML(data.declaration || data, data.suivre || {});
        document.getElementById('declarationsList').classList.add('hidden');
        container.classList.remove('hidden');
    ...
    return `
        <div class="detail-card">
            ...
            <div class="suivi-info">
                ...
                <div class="etape">
                    <i class="fas fa-circle" style="color: ${status.color}"></i>
                    <span>${suivre.etapeActuelle || 'En cours de traitement'}</span>
                </div>
                <div class="prochaine-action">
                    <i class="fas fa-arrow-right"></i>
                    <span>${suivre.prochaineAction || 'En attente de mise à jour'}</span>
                </div>
                ${suivre.estTerminee ? 
                    '<p class="success"><i class="fas fa-check-circle"></i> Cette collecte est terminée</p>' : 
                    ''}
...
function loadProfile() {
    if (!currentUser) {
        console.log('loadProfile: aucun utilisateur connecté');
        return;
    }
    ...
    try {
        ...
        if (currentUser.cree_le) {
            try {
                ...
                document.getElementById('profileSince').textContent = 'Date invalide';
            }
        } else {
            document.getElementById('profileSince').textContent = 'Non disponible';
        }
        ...
        const statusElement = document.getElementById('profileStatus');
        if (statusElement) {
            const isActif = currentUser.estActif === true || 
            ...
            if (isActif) {
                ...
                statusElement.textContent = currentUser.statut || currentUser.status || 'Actif';
                statusElement.className = 'info-value badge badge-success';
            }
        }
        ...
        if (tokenElement) {
            if (currentToken) {
                tokenElement.textContent = currentToken.length > 50 ? 
                    currentToken.substring(0, 50) + '...' : 
                    currentToken;
                console.log('Token affiché dans le profil');
            } else {
                tokenElement.textContent = 'Non connecté';
                console.log('Pas de token à afficher');
            }
        }
...
async function handleUpdateProfile(e) {
    ...
    try {
        ...
        showSuccess(messageDiv, data.message || 'Profil mis à jour avec succès!');
        loadProfile();
...
async function handleChangePassword(e) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Erreur de changement' }));
            throw new Error(errorData.message || 'Erreur de changement');
        }
...
async function loadDashboard() {
    ...
    try {
        ...
        if (!response.ok) {
            console.log('Dashboard non disponible, utilisation fallback');
        ...
        document.getElementById('dashboardPoints').textContent = dashboardData.points || currentUser?.points || '0';
        document.getElementById('dashboardDeclarations').textContent = dashboardData.total_declarations || '0';
        document.getElementById('dashboardCollections').textContent = dashboardData.total_collectes || '0';
        
        // Gestion de total_dechets_collectes
        let totalWaste = dashboardData.total_dechets_collectes;
        if (totalWaste !== undefined && totalWaste !== null) {
            if (typeof totalWaste === 'string') {
                totalWaste = parseFloat(totalWaste);
            }
        ...
        try {
            ...
            if (historyResponse.ok) {
                const historyData = await historyResponse.json();
                const historique = historyData.historique || historyData.data || historyData;
                loadRecentHistory(historique);
            }
        } catch (e) {
            console.log('Historique non disponible');
        }
        
        // Charger les notifications (optionnel)
        try {
            const notificationsResponse = await fetch(`${CONFIG.API_URL}/api/dashboard/notifications?limite=5`, {
                headers: { 'Authorization': `Bearer ${currentToken}` }
            });
            ...
            if (notificationsResponse.ok) {
                const notificationsData = await notificationsResponse.json();
                const notifications = notificationsData.notifications || notificationsData.data || notificationsData;
...
async function loadDashboardFallback() {
    ...
    try {
        ...
        console.error('Erreur fallback dashboard:', error);
    }
}
...
function loadRecentNotifications(notifications) {
    ...
    notifications.forEach(notification => {
        ...
        const timeAgo = getTimeAgo(new Date(notification.cree_le || notification.date || Date.now()));
        
        item.innerHTML = `
            <div class="notification-header">
                <span class="notification-title">${notification.titre || notification.title || 'Notification'}</span>
                <span class="notification-time">${timeAgo}</span>
            </div>
            <div class="notification-message">${notification.message || notification.texte || ''}</div>
        `;
...
function loadRecentHistory(declarations) {
    ...
    declarations.forEach(declaration => {
        ...
        const date = declaration.date_declaration || declaration.date 
            ? new Date(declaration.date_declaration || declaration.date).toLocaleDateString('fr-FR')
            : 'Date inconnue';
        const type = formatWasteType(declaration.type_dechet || declaration.type);
        const quantity = `${declaration.quantite || declaration.quantite || 0} ${declaration.unite || declaration.unit || ''}`;
        const status = declaration.statut || declaration.status || 'inconnu';
...
// Gestion de la session - CORRIGÉ
function saveAuthData(token, user) {
    if (!token || !user) {
        console.error('saveAuthData: données invalides', { token, user });
        return;
    }
...
function loadUserFromStorage() {
    ...
    try {
        ...
        if (!userJson || userJson === 'undefined' || userJson === 'null') {
            console.log('User JSON invalide, nettoyage...');
            localStorage.removeItem(CONFIG.TOKEN_KEY);
            localStorage.removeItem(CONFIG.USER_KEY);
            return;
        }
        ...
        try {
            ...
            console.log('Utilisateur parsé avec succès:', user);
...
function handleLogout() {
    ...
    // Réinitialiser l'affichage du token
    const tokenElement = document.getElementById('currentToken');
    if (tokenElement) {
        tokenElement.textContent = 'Non connecté';
    }
