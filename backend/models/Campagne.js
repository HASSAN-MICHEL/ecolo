...
//                 types_dechets, zones_intervention, poids_attendue,
//                 prix_par_kg, statut, createur_id, createur_type
//             ) VALUES ($1, $2, $3, $4, $5::type_dechet[], $6, $7, $8, $9, $10, $11)
//             RETURNING *
//         `;
...
//         const requete = 'SELECT * FROM campagnes WHERE id = $1';
...
//         let requete = `
//             SELECT c.*, 
//                    COALESCE((SELECT SUM(poids_collecte) FROM suivi_campagne WHERE campagne_id = c.id), 0) as poids_actuel,
//                    c.poids_attendue - COALESCE((SELECT SUM(poids_collecte) FROM suivi_campagne WHERE campagne_id = c.id), 0) as poids_restant
//             FROM campagnes c
//             WHERE 1=1
//         `;
//         const valeurs = [];
...
//         if (filtres.statut) {
//             requete += ` AND c.statut = $${valeurs.length + 1}`;
//             valeurs.push(filtres.statut);
//         }
//         if (filtres.createurId) {
//             requete += ` AND c.createur_id = $${valeurs.length + 1}`;
//             valeurs.push(filtres.createurId);
//         }
//         if (filtres.createurType) {
//             requete += ` AND c.createur_type = $${valeurs.length + 1}`;
//             valeurs.push(filtres.createurType);
//         }

//         requete += ' ORDER BY c.cree_le DESC';
...
//     static async ajouterPromoteur(campagneId, promoteurId, promoteurType, contribution = null, objectif = null) {
//         // Vérifier si ECOCOLLECT est déjà promoteur
//         if (promoteurType === 'ecocollect') {
//             const existant = await pool.query(`
//                 SELECT * FROM promoteurs_campagne 
//                 WHERE campagne_id = $1 AND promoteur_type = 'ecocollect'
//             `, [campagneId]);
            
//             if (existant.rows.length > 0) {
//                 throw new Error('ECoCOLLECT est déjà promoteur de cette campagne');
//             }
//         }

//         const requete = `
//             INSERT INTO promoteurs_campagne (campagne_id, promoteur_id, promoteur_type, contribution_financiere, objectif_specifique)
//             VALUES ($1, $2, $3, $4, $5)
//             ON CONFLICT (campagne_id, promoteur_id, promoteur_type) 
//             DO UPDATE SET 
//                 contribution_financiere = EXCLUDED.contribution_financiere,
//                 objectif_specifique = EXCLUDED.objectif_specifique
//             RETURNING *
//         `;
//         const resultat = await pool.query(requete, [campagneId, promoteurId, promoteurType, contribution, objectif]);
//         return resultat.rows[0];
//     }
...
//             WHERE campagne_id = $1
//             ORDER BY 
//                 CASE WHEN promoteur_type = 'ecocollect' THEN 0 ELSE 1 END,
//                 date_ajout
//         `;
...
//         // Empêcher de retirer ECOCOLLECT si c'est le seul promoteur
//         if (promoteurType === 'ecocollect') {
//             const promoteurs = await this.getPromoteurs(campagneId);
//             if (promoteurs.length <= 1) {
//                 throw new Error('Impossible de retirer le dernier promoteur (ECoCOLLECT)');
//             }
//         }
...
//     static async ajouterSuivi(donnees) {
//         const requete = `
//             INSERT INTO suivi_campagne (campagne_id, date_suivi, poids_collecte, montant_utilise, points_concernes, details)
//             VALUES ($1, $2, $3, $4, $5, $6)
//             ON CONFLICT (campagne_id, date_suivi) 
//             DO UPDATE SET 
//                 poids_collecte = EXCLUDED.poids_collecte,
//                 montant_utilise = EXCLUDED.montant_utilise,
//                 points_concernes = EXCLUDED.points_concernes,
...
//         const requete = `
//             WITH suivi_global AS (
//                 SELECT 
//                     COALESCE(SUM(poids_collecte), 0) as poids_total,
//                     COALESCE(SUM(montant_utilise), 0) as montant_total,
//                     COUNT(DISTINCT date_suivi) as jours_suivis,
//                     COUNT(DISTINCT (details->>'point_id')::uuid) as points_actifs
//                 FROM suivi_campagne
//                 WHERE campagne_id = $1
//             ),
//             suivi_par_point AS (
//                 SELECT 
//                     (details->>'point_id')::uuid as point_id,
//                     SUM(poids_collecte) as poids_point
//                 FROM suivi_campagne
//                 WHERE campagne_id = $1
//                 GROUP BY (details->>'point_id')::uuid
//                 ORDER BY poids_point DESC
//                 LIMIT 5
//             )
//             SELECT 
//                 c.*,
...
//                 sg.jours_suivis,
//                 sg.points_actifs,
//                 c.poids_attendue - sg.poids_total as poids_restant,
//                 CASE 
//                     WHEN c.poids_attendue > 0 
//                     THEN (sg.poids_total / c.poids_attendue * 100)
//                     ELSE 0 
//                 END as pourcentage_atteint,
//                 (SELECT json_agg(json_build_object('point_id', point_id, 'poids', poids_point)) FROM suivi_par_point) as top_points
//             FROM campagnes c
//             CROSS JOIN suivi_global sg
//             WHERE c.id = $1
//         `;
//         const resultat = await pool.query(requete, [campagneId]);
//         return resultat.rows[0];
...
//         const campagne = await this.getStatistiques(campagneId);
//         const promoteurs = await this.getPromoteurs(campagneId);
//         const suiviQuotidien = await this.getSuivi(campagneId);
        
//         const evolution = await pool.query(`
//             SELECT 
//                 date_suivi,
//                 poids_collecte,
//                 SUM(poids_collecte) OVER (ORDER BY date_suivi) as poids_cumule
//             FROM suivi_campagne
//             WHERE campagne_id = $1
//             ORDER BY date_suivi
//         `, [campagneId]);
