...
class GestionnairePoint {
    ...
    try {
        ...
        const verification = await client.query(`
            SELECT m.*, m.collecteur_id, m.declaration_id
            FROM missions m
        ...
        // ✅ METTRE À JOUR LA DÉCLARATION (reste à 'termine' ou 'validee')
        // La déclaration reste à 'termine' car c'est déjà fait
        ...
        if (mission.collecteur_id) {
            ...
            await client.query(`
                ...
                SET gains_total = COALESCE(gains_total, 0) + $1,
                    points_total = COALESCE(points_total, 0) + $2
                WHERE id = $3
...
//  Tableau de bord complet pour un gestionnaire spécifique
static async tableauBord(gestionnaireId) {
    // D'abord récupérer le point de collecte du gestionnaire
    ...
    const pointCollecteId = gestionnaire.point_collecte_id;
    
    const requete = `
        WITH stats AS (
            SELECT 
                COUNT(*) FILTER (WHERE m.statut = 'deposee' AND m.point_depot_id = $1) as en_attente,
                COUNT(*) FILTER (WHERE m.statut = 'validee' AND m.point_depot_id = $1) as validees,
                COALESCE(SUM(m.poids_depose) FILTER (WHERE m.statut = 'validee' AND m.point_depot_id = $1), 0) as poids_total,
                COALESCE(SUM(m.gains_attribues) FILTER (WHERE m.statut = 'validee' AND m.point_depot_id = $1), 0) as gains_distribues,
                COUNT(DISTINCT m.collecteur_id) FILTER (WHERE m.statut = 'validee' AND m.point_depot_id = $1) as collecteurs_actifs
            FROM missions m
            WHERE 1=1
        ),
        missions_recentes AS (
            ...
            JOIN declarations_dechets d ON m.declaration_id = d.id
            JOIN collecteurs c ON m.collecteur_id = c.id
            WHERE m.statut = 'validee' AND m.point_depot_id = $1
            ORDER BY m.date_validation DESC
            LIMIT 10
        ),
        top_collecteurs AS (
    ...
    const resultat = await pool.query(requete, [pointCollecteId]);
    return resultat.rows[0] || {
        statistiques: {
            en_attente: 0,
            validees: 0,
