...
//                 LEFT JOIN missions c ON dd.id = c.declaration_id AND c.statut = 'terminee'
//                 WHERE p.id = $1
//                 GROUP BY p.id
//             `;
...
class DashboardController {
        ...
        try {
            ...
            const requete = `
                SELECT 
                    p.points,
                    COUNT(DISTINCT dd.id) as total_declarations,
                    COUNT(DISTINCT m.id) as total_collectes,
                    COALESCE(SUM(m.poids_depose), 0) as total_dechets_collectes,
                    MAX(m.date_validation) as derniere_collecte_date
                FROM producteurs p
                LEFT JOIN declarations_dechets dd ON p.id = dd.producteur_id
                LEFT JOIN missions m ON dd.id = m.declaration_id AND m.statut = 'validee'
                WHERE p.id = $1
                GROUP BY p.id
            `;
    ...
    static async obtenirNotifications(req, res) {
        try {
            ...
            const requete = `
                SELECT * FROM notifications
                WHERE utilisateur_id = $1 AND type_utilisateur = 'producteur'
                ORDER BY cree_le DESC
                LIMIT $2
            `;
