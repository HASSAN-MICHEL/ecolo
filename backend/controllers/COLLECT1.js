 ...
 static async connexion(req, res) {
    ...
    static async mesMissions(req, res) {
        try {
            ...
            const missions = await Collecteur.obtenirMissions(collecteurId, statut);
    ...
    static async mesGains(req, res) {
        try {
            const collecteurId = req.utilisateurId;
            const gains = await Collecteur.obtenirGains(collecteurId);

            const totalGains = gains.reduce((sum, g) => sum + parseFloat(g.montant), 0);
            const gainsValides = gains.filter(g => g.statut === 'valide')
    ...
    //             success: true,
    //             dashboard,
    //             missionEnCours: missionsEnCours[0] || null
    //         });
    //     } catch (erreur) {
